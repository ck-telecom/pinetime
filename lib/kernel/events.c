...
#if EVENT_DEBUG
static void prv_queue_dump(struct k_msgq * queue) {
  PebbleEvent event;
  PBL_LOG(LOG_LEVEL_DEBUG, "Dumping queue:");
  while (k_msgq_get(queue, &event, K_NO_WAIT) == 0) {
...
struct k_msgq * event_get_to_kernel_queue(PebbleTask task) {
...
static bool prv_event_put_isr(struct k_msgq * queue, const char* queue_type, uintptr_t saved_lr,
                                  PebbleEvent* event) {
  // PBL_ASSERTN(queue);

  int result = k_msgq_put(queue, event, K_NO_WAIT);
...
static bool prv_try_event_put(struct k_msgq * queue, PebbleEvent *event) {
  // PBL_ASSERTN(queue);
  return (k_msgq_put(queue, event, K_MSEC(3000)) == 0);
}

static void prv_event_put(struct k_msgq * queue,
                          const char* queue_type,
                          uintptr_t saved_lr,
                          PebbleEvent* event) {
  // PBL_ASSERTN(queue);

  if (k_msgq_put(queue, event, K_MSEC(3000)) != 0) {
...
void event_put(PebbleEvent* event) {
  ...
  if (pebble_task_get_current() == PebbleTask_KernelMain) {
    ...
    prv_event_put(&s_kernel_event_queue, "kernel", saved_lr, event);
  }
}
...
void event_put_from_process(PebbleTask task, PebbleEvent* event) {
  ...
  struct k_msgq * queue = event_get_to_kernel_queue(task);
...
bool event_take_timeout(PebbleEvent* event, int timeout_ms) {
  ...
  struct k_poll_event local_poll_events[4];
  memcpy(local_poll_events, poll_events, sizeof(poll_events));

  result = k_poll(local_poll_events, 4, K_MSEC(timeout_ms));
  if (result != 0) {
...
//    WTF;
  ...
  event_queue_cleanup_and_reset((struct k_msgq *)reset_queue);

  // In Zephyr, we don't need to manage queue sets, so no additional steps are needed
}


struct k_msgq *event_kernel_to_kernel_event_queue(void) {
  return &s_from_kernel_event_queue;
}

int event_queue_cleanup_and_reset(struct k_msgq *queue) {
  PebbleEvent event;
