...
#endif
    return 0;//lvgl_indev_init((void *)touch_dev);
}
SYS_INIT(touch_init, APPLICATION, 91);

static void touch_thread()
{
    const struct device *dev = device_get_binding(TOUCH_DEV);
    if (!dev) {
    ...
    while (1) {
        ...
        sensor_channel_get(dev, CST816S_CHAN_STATE, &state);
        LOG_INF("Gesture %d on x=%d, y=%d", state.val1, touch_point.val1, touch_point.val2);
    }
}
